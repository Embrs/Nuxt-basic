...
module.exports = class GenLocalesWebpackPlugin {
  ...
  apply(compiler) {
    compiler.hooks.afterPlugins.tap("GenLocalesWebpackPlugin", () => {
      // check has locale key
      if (!this.has(this.source, "locale"))
      ...
      locales.forEach((locale, index) => {
        ...
        fs.writeFileSync(
          file,
          `/* eslint-disable */  export default (${result})`
        );
      });
    });
  }
  ...
  init() {
    ...
    const source = fs.readFileSync(
      `${this.options.path}/${this.options.target}`,
      "utf-8"
    );
